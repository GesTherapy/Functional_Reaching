...
namespace Microsoft.Samples.Kinect.BasicInteractions
{
    ...
    /* ADDED BY GESTHERAPY */ using Coding4Fun.Kinect.Wpf;
    ...
    public class KinectController : INotifyPropertyChanged, IDisposable
    {
        ...
        {
            /* ADDED BY GESTHERAPY */
            Joint scaledJoint = joint.ScaleTo((int)SystemParameters.PrimaryScreenWidth, (int)SystemParameters.PrimaryScreenHeight, .7f, .5f);
            double y = scaledJoint.Position.Y;
            return y;
        ...
        private static void ScaleXY(Joint shoulderCenter, bool rightHand, Joint joint, out int scaledX, out int scaledY)
        {
            ...
            double x = scaledJoint.Position.X;
            double y = scaledJoint.Position.Y;
            if (x < 0)
                x = 0;
